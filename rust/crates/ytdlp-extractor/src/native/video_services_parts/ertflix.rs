        ...
        if video_id.starts_with("ser.") {
            ...
            if !season_numbers.is_empty() {
                if let Some(seasons) = series.get("Seasons").and_then(serde_json::Value::as_array) {
                    for season in seasons {
                        if season
                            .get("SeasonNumber")
                            .and_then(|_| json_i64(season, "SeasonNumber"))
                            .is_some_and(|number| season_numbers.contains(&number))
                        {
                            if let Some(title) = json_string(season, "Title")
                                .filter(|value| !value.is_empty())
                            {
                ...
                .and_then(serde_json::Value::as_array)
            {
                for group in groups {
                    ...
                    if !allowed_titles.is_empty() && !allowed_titles.iter().any(|title| title == group_title)
                    {
                        continue;
                    }
                    let Some(episodes) = group.get("Episodes").and_then(serde_json::Value::as_array)
                    else {
                        continue;
                    };
                    ...
                    if all_numbered {
                        ordered.sort_by_key(|episode| json_i64(episode, "EpisodeNumber").unwrap_or_default());
                    }
...
fn ertflix_image_values<'a>(value: &'a serde_json::Value, output: &mut Vec<&'a serde_json::Value>) {
    match value {
    ...
    for media in media_files {
        ...
        for source in media_formats {
            ...
            let mut format = serde_json::json!({
                "url": media_url,
                "format_id": if is_hls { "hls".to_owned() } else if is_dash { "dash".to_owned() } else {
                    json_value_string(source.get("Id")).unwrap_or_else(|| "http".to_owned())
                },
                "protocol": if is_hls { "m3u8_native" } else if is_dash { "http_dash_segments" } else { "http" },
