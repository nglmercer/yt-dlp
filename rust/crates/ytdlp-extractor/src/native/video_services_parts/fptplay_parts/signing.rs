...
fn fptplay_api_url(video_id: &str, episode: i64) -> String {
    ...
    format!(
        "https://api.fptplay.net{path}?st={token}&e={timestamp}"
    )
}

fn fptplay_token(message: &str) -> String {
    fptplay_base64_urlsafe(&fptplay_md5(message.as_bytes()))
}
...
mod fptplay_signing_tests {
    ...
    fn token_uses_url_safe_unpadded_base64() {
        assert_eq!(
            fptplay_base64_urlsafe(&fptplay_md5(b"abc")),
